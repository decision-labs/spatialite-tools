...
start_node (const char **attr) 
{
    ...
    current_tag = CURRENT_TAG_IS_NODE;
...
end_node (struct aux_params *params) 
{
    ...
    current_tag = CURRENT_TAG_UNKNOWN;
}

static void 
start_way (const char **attr) 
{
    ...
    current_tag = CURRENT_TAG_IS_WAY;
...
build_linestring (sqlite3 * db_handle) 
{
	  ...
	  strcpy (sql, "SELECT id, lat, lon FROM osm_tmp_nodes ");
...
start_xtag (const char **attr) 
{
    ...
    if (current_tag == CURRENT_TAG_IS_NODE)
    ...
    if (current_tag == CURRENT_TAG_IS_WAY)
    ...
    if (current_tag == CURRENT_TAG_IS_RELATION)
...
start_relation (const char **attr) 
{
    ...
    current_tag = CURRENT_TAG_IS_RELATION;
...
build_multilinestring (sqlite3 * db_handle) 
{
	  ...
	  strcpy (sql, "SELECT id, Geometry FROM osm_tmp_ways ");
...
build_multipolygon (sqlite3 * db_handle) 
{
	  ...
	  strcpy (sql, "SELECT id, area, Geometry FROM osm_tmp_ways ");
...
start_tag (void *data, const char *el, const char **attr) 
{
	...
	data = NULL;		/* suppressing stupid compiler warnings about unused args */
	...
	db_cleanup (handle);
	...
	
      {
	      ...
	      
	    {
	      ...
	      sqlite3_backup_init (disk_db_handle, "main", handle, "main");
